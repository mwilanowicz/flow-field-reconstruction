...
Parameters reduced by increasing spatial downsampling to prevent overfitting on small datasets.
...
class CAE(nn.Module):
    ...
    def __init__(self, latent_dim=16):
        ...
        self.encoder = nn.Sequential(
            ...
            ConvBlock(32, 32, stride=2), # 32 x 32 x 64 -> 32 x 16 x 32
            ConvBlock(32, 32, stride=2), # 32 x 16 x 32 -> 32 x 8 x 16
            nn.Flatten(), # 32 x 8 x 16 = 32 * 8 * 16 = 4096 values (flattening 3D tensor to 1D vector)

            # Compression into latent space z: 4096 -> 16
            nn.Linear(32 * 8 * 16, latent_dim) 
        )

        self.decoder = nn.Sequential(
            # Decompression from latent space z: 16 -> 4096
            nn.Linear(latent_dim, 32 * 8 * 16),
            nn.Unflatten(1, (32, 8, 16)), # Reshaping values back to tensor (Channels, Height, Width)

            nn.Upsample(scale_factor=2, mode='nearest'),
            ConvBlock(32, 32), # 32 x 8 x 16 -> 32 x 16 x 32

            nn.Upsample(scale_factor=2, mode='nearest'),
            ConvBlock(32, 32), # 32 x 16 x 32 -> 32 x 32 x 64
